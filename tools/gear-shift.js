...
var bassPatternTable = probable.createTableFromSizes([
  [8, theme],
  [3, chug],
  [1, arpeggioUp],
  //[1, runDown],
...
var rhythmPatternTable = probable.createTableFromSizes([
  [1, runUp],
  //[1, runDown],
  //[1, arpeggioUp],
  [1, arpeggioDown],
...
for (let sectionIndex = 0; sectionIndex < sectionCount; ++sectionIndex) {
  ...
  let { rhythmSection, leadSection, bassSection } = tracksForSection({
  ...
  bassTrack = bassTrack.concat(bassSection);
}

// TODO: Write out two files.
var eventsTrack;
if (role === 'rhythm') {
  eventsTrack = rhythmTrack;
} else if (role === 'lead') {
  eventsTrack = leadTrack;
} else {
  eventsTrack = bassTrack;
}
...
function tracksForSection({ sectionBarCount, sectionRoot, sectionMode }) {
  ...
  var bassSection = measureModeDegrees.map(eventsForBassBar).flat();
  return { rhythmSection, leadSection, bassSection };

  // The tonal center is going to be at degreeRoot + the offset, but the root
  // of the mode is still sectionRoot.
  function eventsForRhythmBar(degreeRoot) {
    return rhythmPatternTable.roll()({
      root: sectionRoot + 12, 
      mode: sectionMode,
  ...
  function eventsForBassBar(degreeRoot) {
    return bassPatternTable.roll()({
      root: sectionRoot + 12, 
      mode: sectionMode,
      startDegree: degreeRoot,
      beats: 4
...
// Warning: Always produces four beats.
function theme({ root, mode, beats }) {
  if (beats !== 4) {
    return [];
  }
  ...
  return [
    ...
    .map(({ p, d}) => 
      notePair({
        creator: 'theme',
        mode: mode.name,
        length: 2 * sixteenthNoteTicks * d,
        noteNumber: getPitchInMode(root, p, mode),
        velocity: probable.roll(32) + 48 + (p === 0 ? 32 : 0),
      })
    ).flat();
}

function chug({ root, mode, beats }) {
  return range(beats * 2).map(i => 
    notePair({
      creator: 'chug',
      mode: mode.name,
      length: 2 * sixteenthNoteTicks,
      noteNumber: getPitchInMode(root, 0, mode),
      velocity: probable.roll(32) + 48 + (i === 0 ? 32 : 0),
    })
  ).flat();
}
